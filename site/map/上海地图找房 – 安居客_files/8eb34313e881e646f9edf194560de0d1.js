...
(function () {
    ...
    //导航登陆逻辑
    window.loginObj = new J.ui.login({baseUrl:'',showSimple:false});
...
function LoginDialog(option, regster_data) {
    var defOptions = {
        url: 'http://shanghai.release.lunjiang.dev.anjuke.com',//当前ＵＲＬ，需要base64编码
        memberDomain: 'http://member.lunjiang.dev.anjuke.com',
        userDomain: 'http://user.release.lunjiang.dev.anjuke.com',
            ...
            dom.autocomplete({
                ...
                onSelect: function (data) {
                    ...
                    if (submitType == 'mail') {
                        var mail = J.g("b_mail");
                        mail.hide();
                        mail.s(".v_error").eq(0).hide();
                        mail.s("label").eq(0).html("验证码：");
                        //  J.get({type:'jsonp',url:opts.validateSuccessMail,data:{action:"ajax_times",r:Math.random(),callback:"global.loginDailog.callbackMailOver"}});
                        global.loginDailog.callbackMailValidate = checkMail;
            ...
            function clickHandler() {
                global.loginDailog.sendVerifyCodeTel = function (data) {//发送手机验证码
                    ...
                    if (data.result == "success") {
                        ...
                        mail.hide();
                        dom_info.html("请输入手机号码").show();
                    }
                    else if(data.result=="error" && data.code == -1){

                        J.site.trackEvent('over_verify_d');
                        mail.s("label").eq(0).html("校验码：");
                        mail.s(".v_error").eq(0).html("校验码不能为空").show();
                        mail.show();
                    }
                    else if(data.result=="error" && data.code == -2){
                        J.site.trackEvent('over_verify_d');

                        mail.s("label").eq(0).html("校验码：");
                        mail.s(".v_error").eq(0).html("校验码有误").show();
                        mail.s("a").eq(0).get().click();
                        mail.show();
                    }
                    else {
                        mail.hide();
                ...
                J.get({type: 'jsonp',
                    url: opts.userDomain + "/register",
                    data: {
                        chktype: 'verifyformat',
                        r: Math.random(),
                        phone: J.g("iptUserName").val(),
                        referer: opts.url,
                        code: J.g("b_mail").s("input").eq(0).val()
                    },
